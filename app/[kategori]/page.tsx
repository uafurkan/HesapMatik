  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 md:py-16">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 mb-10 animate-fade-in">
        <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-white/10 to-white/5 border border-white/10 flex items-center justify-center text-4xl sm:text-5xl shadow-lg shrink-0">
          {cat.icon}
        </div>
        <div>
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black font-syne text-white tracking-tight">{cat.name} <span className="gradient-text block sm:inline">Hesaplamaları</span></h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mt-10">
        {cat.hesaplamalar.map((h, i) => (
          <Link key={h.slug} href={`/${cat.slug}/${h.slug}`} className="glass-card rounded-2xl p-5 sm:p-6 hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(255,179,71,0.15)] hover:border-amber-500/50 transition-all duration-300 flex flex-col h-full group animate-slide-up" style={{ animationDelay: `${i * 0.05}s` }}>
            <h3 className="text-lg sm:text-xl font-bold text-gray-100 mb-3 font-syne group-hover:text-amber-400 transition-colors">{h.title}</h3>
            <p className="text-xs sm:text-sm text-gray-400 font-mono flex-grow leading-relaxed">{h.description || `${h.title} hesaplayıcısını hemen kullanın.`}</p>
