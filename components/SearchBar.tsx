  ...
  return (
    <div className="relative max-w-2xl mx-auto my-8 z-50">
      ...
      {query.length > 1 && isFocused && (
        <div className="absolute top-full left-0 right-0 mt-3 glass-card rounded-2xl overflow-hidden z-50 animate-slide-up border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
          {results.length > 0 ? (
            <ul className="max-h-80 overflow-y-auto">
              {results.map((r, i) => (
                <li key={i} className="border-b border-white/5 last:border-none">
                  <Link href={`/${r.kategori}/${r.slug}`} className="block p-4 hover:bg-white/5 transition-colors group">
                    <div className="font-bold text-gray-200 group-hover:text-amber-400 transition-colors">{r.title}</div>
                    <div className="text-sm text-gray-500 font-mono mt-1 group-hover:text-gray-400">{r.description}</div>
                  </Link>
                </li>
              ))}
            </ul>
